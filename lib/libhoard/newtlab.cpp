...
public:
  ...
  inline size_t getSize (void * obj) const {
    // Unfortunately, for some reason malloc_usable_size is not
    // being exported. Have to talk to Doug about this.
    // So in lieu of said function, we directly muck with the object
    // header to grab its size. Ugly, but necessary.
    return LeaHeader::getSize (obj);
  }
