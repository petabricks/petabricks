...
public:
  ...
  void put (void * s, size_t sz) {
    ...
    //    printf ("global heap put %x, size %d\n", s, sz);
    theHeap->put ((typename SuperHeap::SuperblockType *) s,
		  sz);
  }
  ...
  SuperblockType * get (size_t sz, void * dest) {
    SuperblockType * s = 
      ...
      (theHeap->get (sz, reinterpret_cast<SuperHeap *>(dest)));
    if (s) {
      assert (s->isValidSuperblock());
    }
    //    printf ("global heap get %x, size %d\n", s, sz);
    return s;
  }
