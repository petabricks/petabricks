...
template <class SourceHeap,
	  ...
	  class LockType,
          ...
          class HeapType>
class HoardManager : public BaseHoardManager<SuperblockType_, SuperblockSize>,
                     public thresholdFunctionClass
{
private:

  typedef BaseHoardManager<SuperblockType_, SuperblockSize> SuperHeap;
  ...
  HL::sassert<(sizeof(SuperblockType_) == SuperblockSize)> verifySize; 
...
public:
  ...
  INLINE void free (void * ptr) {
    ...
    if ((thresholdFunctionClass::function (u, a, sz))) {
      // (s->getTotalObjects() == s->getObjectsFree())) {
...
private:
  ...
  NO_INLINE void slowPathFree (int binIndex, int u, int a) {
    ...
    if (sb) {
      ...
      const size_t sz = sb->getObjectSize();
      Statistics& stats = _stats(binIndex);
  ...
  void addStatsSuperblock (SuperblockType * s, int binIndex) {
    ...
    //    printf ("u = %d, totalObjects = %d, s->getObjectsFree() = %d\n", u, totalObjects, s->getObjectsFree());
  ...
  MALLOC_FUNCTION NO_INLINE void * slowPathMalloc (size_t sz) {
    ...
    for (;;) {
      Check<HoardManager, sanityCheck> check (this);
      void * ptr = getObject (binIndex, sz);
      if (ptr) {
	return ptr;
      } else {
	Check<HoardManager, sanityCheck> check (this);
	// Return null if we can't allocate another superblock.
	if (!getAnotherSuperblock (sz)) {
...
private:
  ...
  HL::Array<NumBins, Statistics> _stats;

  typedef SuperblockType * SuperblockTypePointer;

  typedef EmptyClass<SuperblockType, EmptinessClasses, SuperblockSize> OrganizedByEmptiness;

  typedef ManageOneSuperblock<OrganizedByEmptiness> BinManager;

  /// Bins that hold superblocks for each size class.
  HL::Array<NumBins, BinManager> _otherBins;
