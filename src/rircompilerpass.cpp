...
void petabricks::ExpansionPass::before(RIRStmtCopyRef& s){
  ...
  if(s->type() == RIRNode::STMT_LOOP && s->hasAnnotation("for_enough")){
    ...
    _transform.addConstant(INPUT_SIZE_STR);//force input size to be passed to us

    // insert some code before the for_enough loop
    t = RIRStmt::parse("int "+vCount+"= petabricks::interpolate_iteration_count("+_transform.name()+"_"+config+", "INPUT_SIZE_STR", "+jalib::XToString(minI)+" );");
    t->accept(*this);
    pushStmtBackward(t);
