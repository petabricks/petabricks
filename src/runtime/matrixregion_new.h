...
public:
  ...
  /*INLINE ElementT* cellPtr(const IndexT c1[D]) const{ return this->coordToPtr(c1); }
  INLINE IndexT intervalBytes(const IndexT c1[D], const IndexT c2[D]) const 
  { ssize_t s=1;
    for(int i=0; i<D; ++i) {
      s*=c2[i] - c1[i];
    }
    return s*sizeof(ElementT);
  }*/
  ...
  /// return largest dimension incremented or -1 for end
  int incCoordWithBound(IndexT coord[D], IndexT c1[D], IndexT c2[D]) const{
    if(D==0) 
     return -1;
    int i;
    coord[0]++;
